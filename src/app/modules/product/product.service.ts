...
const getAll = async (filters: IProductFilterRequest, options: IPaginationOptions): Promise<IGenericResponse<Product[]>> => {
    ...
    const result = await prisma.product.findMany({ include: { attribute_group: true, category: true, meta_SEO: true, }, where: whereConditions, skip, take: limit, orderBy: options.sortBy && options.sortOrder ? { [options.sortBy]: options.sortOrder } : { created_at: "desc" } })
